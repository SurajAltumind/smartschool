...
// print_r($onlineexam);
    ...
    public function getsyllabusOLD()
    {
    ...
    public function getsyllabus()
    {
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    // $subject_group_subject_id=134;
                    // $subject_group_class_sections_id=49;
                    //  $time_from = '10:30 AM';
                    //  $time_to= '11:00 AM';
                    //   $date= '1970-01-01';
     ...
     public function getsyllabusDev()
    {
    ...
    // public function getExamResultList3()
    // {
    //     $method = $this->input->server('REQUEST_METHOD');
    //     if ($method != 'POST') {
    //         json_output(400, array('status' => 400, 'message' => 'Bad request.'));
   ...
   public function multiclassstudent()
        ...
        else 
        {
            ...
            if ($check_auth_client == true) 
            {
                ...
                if ($response['status'] == 200) 
                {
                   ...
                   // $student_id = 1449;
                    // $keyword = $params['keyword'];
                       $student_id = $this->input->get_request_header('student_id');
    ...
    ///*************************New Multiclass******************************//
      public function multiclassstudentone()
    {
        $method = $this->input->server('REQUEST_METHOD');
        if ($method != 'POST') 
        {
        ...
        {
            $check_auth_client = $this->auth_model->check_auth_clientone();
            if ($check_auth_client == true) 
            {
                $response = $this->auth_model->authone();
                if ($response['status'] == 200) 
                {
                    $data = array();
                    $params     = json_decode(file_get_contents('php://input'), true);
    ...
    //*******************Live Class New API**********************************//
     public function liveclassesone()
    {
        $method = $this->input->server('REQUEST_METHOD');
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                       $_POST        = json_decode(file_get_contents("php://input"), true);
                    $student_id   = $this->input->post('student_id');
      $exam_subjects = $this->examschedule_model->getresultByStudentandExam($student_id);
                    $data["exam_result_list"] = $exam_subjects;
                    json_output($response['status'], $data);
                }
            }
        }
