            ...
            if ($this->form_validation->run() == false) {
                ...
                if (isset($search)) {
                    if ($search == 'search_filter') {
                        $resultlist         = $this->student_model->searchByClassSectionCategoryGenderRte($class, $section, $category_id, $gender, $rte,$subject);
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->sch_setting_detail->adm_auto_insert) {
               ...
               if(is_array($this->input->post('subjects'))){
                            $data_insert['subjects'] = implode(',',$this->input->post('subjects'));
                
            }else{
                                            $data_insert['subjects'] = $this->input->post('subjects');
                ...
                if ($ext == 'csv') {
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            $student_data[$i]["admission_date"]=date('Y-m-d',strtotime($student_data[$i]["admission_date"]));
                            $student_data[$i]["measurement_date"]=date('Y-m-d',strtotime($student_data[$i]["measurement_date"]));
