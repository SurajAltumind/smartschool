...
public function combinedincome(){

         $this->session->set_userdata('top_menu', 'Reports');

        $this->session->set_userdata('sub_menu', 'Reports/finance');

        $this->session->set_userdata('subsub_menu', 'Reports/finance/combinedincome');
     ...
     $data['incomeList']  = $this->income_model->search("", $start_date, $end_date);
    $data['expenseList']  = $this->expense_model->search("", $start_date, $end_date);
     $data['collectlist'] = $this->studentfeemaster_model->getFeeCollectionReport($start_date, $end_date);
         $data['banklist'] = $this->income_model->searchbank('',$start_date, $end_date,'');


//echo '<pre>';print_r($data['banklist']);die;



       $this->load->view('layout/header',$data);

        $this->load->view('reports/combinedincome',$data);
...
public function transfer_to_bank(){
          ...
          $data['name']='Deposit in bank';
          $data['type']='0';
          $data['date']=date('Y-m-d');

$this->db->insert('bank_transfer_details', $data);
       ...
       $this->load->view('reports/transfer_to_bank',$data);
...
$this->db->insert('bank_transfer_details', $data);
       ...
       $this->load->view('reports/cash_handover',$data);
...
$this->db->insert('bank_transfer_details', $data);
       ...
       $this->load->view('reports/withdraw_from_bank',$data);
