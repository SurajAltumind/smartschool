                                    ...
                                    <div class="padd-en-rtl33"> 
                                        <table class="table mb0 font13" >
                                            ...
                                            foreach ($monthAttendance as $attendence_key => $attendence_value) {
                                                if($current_leave==''){$current_leave=2*($attendence_value['absent'])+$attendence_value['half_day'];
                                                }
                                                ?><tr>
                                    ...
                                    <div class="sameheight">
                                        <div class="feebox">
                                            <table class="table3" id="tableID2">
                                              <?php  $deductions = $this->payroll_model->getall_deductions($month, $year, $staff_id);
                                              $staffLeaveDetails = $this->staff_model->getLeaveDetails($staff_id);
                                              $allowedholiday=0;
                                              foreach($staffLeaveDetails as $staffLeaves){
                                                if($staffLeaves['type']=='CL'||$staffLeaves['type']=='PL'){
                                                    $allowedholiday=$allowedholiday+$staffLeaves['alloted_leave'];
                                                }
                                              }
                                         //    $current_leave=5;
                                    //          echo '<pre> holiday ';print_r($allowedholiday);
                                     //          echo '<pre>leave ';print_r($current_leave);
                                             $count=0;
                                                foreach($deductions as $deduction){?>
                                                <tr id="deduction_row<?php echo $count?>">

                                                    <td><input type="text" id="deduction_type" name="deduction_type[]" class="form-control" value="<?php echo $deduction['detail']?>" placeholder="Type"></td>

                                                    <td><input type="text" id="deduction_amount" name="deduction_amount[]" class="form-control"value="<?php echo $deduction['total_deduction']?>"></td>
                                                    <td><button type="button" onclick="delete_deduction_row(<?php echo $count?>)" class="closebtn"><i class="fa fa-remove"></i></button></td>
                                                ...
                                                <?php $count++;}
                                                   if($current_leave>(2*$allowedholiday) ){
                                                    $leaves=$current_leave-(2*$allowedholiday);
                                                    $salary=0;
                                                    if (!empty($result["basic_salary"])) {
                                                        $salary= floor($leaves*$result["basic_salary"]/60);
                                                    }
                                                    ?>
                                                <tr id="deduction_row<?php echo $count?>">

                                                    <td><input type="text" id="deduction_type" name="deduction_type[]" class="form-control" value="Unpaid Leaves <?php echo $leaves/2;?>" placeholder="Type"></td>

                                                    <td><input type="text" id="deduction_amount" name="deduction_amount[]" class="form-control"value="<?php echo $salary?>"></td>
                                                    <td><button type="button" onclick="delete_deduction_row(<?php echo $count?>)" class="closebtn"><i class="fa fa-remove"></i></button></td>
