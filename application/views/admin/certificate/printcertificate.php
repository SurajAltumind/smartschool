    ...
    foreach ($student as $std_key => $std_value) 
    {
		...
		if ($std_key == "admission_date") 
        {
            if ($std_value != "0000-00-00") 
            {
                ...
                $AdmDate = date($this->customlib->getSchoolDateFormat(), $this->customlib->dateYYYYMMDDtoStrtotime($std_value));
       ...
       $certificate_body = str_replace('[' . $std_key . ']', $std_value , $certificate_body);
        ...
        //--certificate number
            ...
            $certificateCount=$certificate[0]->certificate_count;
            $certificateID=$certificate[0]->id;
            $certificate_body=str_replace('[Certi NO]',$certificateCount,$certificate_body);
            $certificateCount++;
            $this->student_model->updateCertificateCount($certificateCount,$certificateID);
       //--certificate number

        //----------session 
           $dur=$student->session;
           $AdmissDate=$student->admission_date;
           $toYearFourDigit=substr($AdmissDate,0,4);
           $oneYearPlus=$toYearFourDigit+1;
           $FromYearLastTwoDigit=substr($oneYearPlus,2);
           $da="-";
           $fromSeeYear=$toYearFourDigit.$da.$FromYearLastTwoDigit;
           
            $session=explode("-",$dur);
            $fromyear=$session[0];
            $toyear=(int)$fromyear+1; 
            $toYearMinusOne= $toyear-1;
            
            $toYearLastTwoDigit=substr($toyear,2);
            $toyear2=$toYearMinusOne.$da.$toYearLastTwoDigit;
            $fromYearMinusfourlastyear=$toyear-4;
            $secondFromYear=$fromYearMinusfourlastyear+1;
            $twoDigitFromYear=substr($secondFromYear,2);
            $fromYear=$fromYearMinusfourlastyear.$da.$twoDigitFromYear;
            ...
            $certificate_body=str_replace('[from_year]', $fromSeeYear, $certificate_body);
            $certificate_body=str_replace('[to_year]', $toyear2, $certificate_body);
        //-----------session 
