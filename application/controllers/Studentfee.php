...
class Studentfee extends Admin_Controller
{
    ...
    public function deleteFee()
    {
        ...
        public function updateFee($student_fees_master_id,$fee_groups_feetype_id,$sub_invoice)
    {
...
//               echo '<pre>';print_r($a);die;
         ...
         redirect('studentfee/addfee/'.$student_fees_master_id);
    }
    ...
    public function addstudentfee()
    {
       ...
       // $this->form_validation->set_rules('amount', $this->lang->line('amount'), 'required|trim|xss_clean|callback_check_deposit');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $accepted=0;
            if('Cheque'==$this->input->post('payment_mode')){
                $accepted=1;
            }
            $json_array               = array(
    ...
    public function printFeesByName()
    {
        ...
        $setting_result      = $this->setting_model->get();
        $data['settinglist'] = $setting_result;
           $data['student_due_fee'] = $this->studentfeemaster_model->getStudentFees($student_session_id);
    ...
    public function printFeesByGroup()
    {
        ...
        $setting_result        = $this->setting_model->get();
        $data['settinglist']   = $setting_result;
          $data['student_due_fee'] = $this->studentfeemaster_model->getStudentFees($fee_master_id);
