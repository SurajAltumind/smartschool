...
class Generatecertificate extends Admin_Controller
{
    ...
    public function downloadReport2($id)
    {
        ...
        $pdfFilePath="downloadReport";
      	$this->pdf->loadHtml($certificates);
		$this->pdf->render();
		$this->pdf->stream("".$pdfFilePath.".pdf",array("Attachment"=>0));
    }
    // custom student report generation
    
         public function StudentAdmissionReport($id)
    {
        $data['students'] = $this->student_model->getCStudentsByID($id);
        $certificates = $this->load->view('admin/certificate/StudentAdmissionReport', $data, true);
        $pdfFilePath = "downloadReport.pdf";
        $this->m_pdf->pdf->WriteHTML($certificates);
