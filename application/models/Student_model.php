...
public function search_alumniStudent($class_id = null, $section_id = null,$session_id = null)
 ...
 public function search_alumniStudentbyAdmissionNo($searchterm, $carray){
    ...
    public function searchByClassSectionName($class_id = null, $section_id = null, $Name = null) 
    {
        $this->db->select('`ipaddress`, `user_agent`, `login_datetime`')->from('userlog');
        // $this->db->join('student_session', 'student_session.student_id = students.id');
        // $this->db->join('classes', 'student_session.class_id = classes.id');
        // $this->db->join('sections', 'sections.id = student_session.section_id');
        // $this->db->join('categories', 'students.category_id = categories.id', 'left');
        $this->db->where('userlog.role', 'Super Admin');
        // $this->db->where('students.is_active', 'yes');
        // if ($class_id != null) {
        //     $this->db->where('student_session.class_id', $class_id);
        // }
    ...
    public function searchByClassSectionWithSession($class_id = null, $section_id = null, $session_id = null)
    {
     ...
     {        
         $this->db->select('summary.score,summary.StartTime,summary.EndTime,summary.TimeSpent,summary.attempts,subject_syllabus.sub_topic,topic.name,subject_syllabus.created_at, classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode ,     students.religion,     students.dob ,students.current_address,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.app_key,students.parent_app_key,students.rte,students.gender,' . $field_variable)->from('students');
         $this->db->join('student_session', 'student_session.student_id = students.id');
         $this->db->join('classes', 'student_session.class_id = classes.id');
         $this->db->join('sections', 'sections.id = student_session.section_id');
     ...
     public function VideoSearchFullText($searchterm, $carray = null)
     {
         ...
         $this->db->select('summary.score,summary.StartTime,summary.EndTime,summary.TimeSpent,summary.attempts,subject_syllabus.sub_topic,topic.name,subject_syllabus.created_at,classes.id AS `class_id`,students.id,student_session.id as student_session_id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode ,     students.religion,     students.dob ,students.current_address,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,      students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code ,students.father_name , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.gender,students.rte,student_session.session_id,' . $field_variable)->from('students');
         $this->db->join('student_session', 'student_session.student_id = students.id');
         $this->db->join('classes', 'student_session.class_id = classes.id');
         $this->db->join('sections', 'sections.id = student_session.section_id');
         $this->db->join('summary', 'summary.user = students.id');
...
$this->db->select('sessions.session AS `session`,students.TC_Certificate_No as `Certificate_No`,students.id AS `id`,classes.id AS `class_id`,student_session.id as `student_session_id`,students.id,classes.class as `class`,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,  students.lastname,students.image,    students.mobileno, students.email ,students.juneJulyClass,students.adhar_no,students.juneJulyWrkDay,students.TotalWorkingDays,students.TotalPresentDay,students.juneJulyPrstDay,students.septNovClass,students.septNovWrkDay,students.septNovPrstDay,students.decFebClass,students.decFebWrkDay,students.decFebPrstDay,students.marMayClass,students.marMayWrkDay,students.marMayPrstDay,students.conduct,students.dateOfLeavingSchool,students.classCourseOn,students.identitymarkone,students.identitymarktwo, students.state ,   students.city , students.pincode ,     students.religion,     students.dob ,students.current_address,students.blood_group ,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.cast,students.bank_name, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.mother_name,students.updated_at,students.father_name,students.rte,students.gender,users.id as `user_tbl_id`,users.username,users.password as `user_tbl_password`,users.is_active as `user_tbl_active`,' . $field_variable)->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
    ...
    //--------------custom function for certifcate count
    // public function updateCertificateNumber($tcCount,$sid)
    // {
    //     $sql="UPDATE `students` SET `TC_Certificate_No`='$tcCount' WHERE `id`='$sid'";
    //     $rs=$this->db->query($sql);
    //     return $rs; 
    // }
    public function updateCertificateCount($Count,$cid)
    {
        $sql="UPDATE `certificates` SET `certificate_count`='$Count' WHERE `id`='$cid'";
        $rs=$this->db->query($sql);
        return $rs; 
    }

    public function getStudentsByID($array)
    {
        $i               = 1;
        $custom_fields   = $this->customfield_model->get_custom_fields('students');
    ...
    public function getCStudentsByID($array)
    {      
         $i               = 1;
        $custom_fields   = $this->customfield_model->get_custom_fields('students');
...
// Execute the query and fetch results
    ...
    public function getAllClassSectionOLD($class_id = null, $section_id = null)
    {
     ...
     public function getAllClassSection($class_id = null, $section_id = null)
    {
     ...
     public function getAllClassSectionDueFees($class_from = null, $class_to = null)
    {

            // $where = array();
          // $where['section_id'] = $section_id;
           return $this->db->select('*')->from('class_sections')->join('classes', 'class_sections.class_id=classes.id', 'inner')->join('sections', 'class_sections.section_id=sections.id', 'inner')->where("class_id BETWEEN '$class_from' AND '$class_to' ")->get()->result_array();
    }
 ...
 public function search_alumniStudentbyAdmissionNoReport($searchterm, $carray){
    ...
    public function activestudentsClass() 
    {
      $this->db->select('classes.class,count(students.category_id) students_cat,categories.category')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        $this->db->where('student_session.session_id', $this->current_session);
        ...
        $this->db->group_by('classes.class');
        $query = $this->db->get();
        return $query->result_array();
    }

    public function activestudentsbyClass($class_id) 
    {
      $this->db->select('classes.class,count(students.category_id) students_cat,categories.category,students.category_id')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        $this->db->where('student_session.session_id', $this->current_session);
        $this->db->where('students.is_active', 'yes');
        $this->db->where('classes.class', $class_id);

        $this->db->group_by('students.category_id');
        $query = $this->db->get();
        return $query->result_array();
    }

    public function activestudents_rte($class_id) 
    {
      $this->db->select('count(students.rte) rte')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->where('student_session.session_id', $this->current_session);
        $this->db->where('students.rte', 'Yes');
        $this->db->where('students.is_active', 'yes');
        $this->db->where('classes.class', $class_id);
    ...
    public function activestudents_male($class_id) 
    {
      $this->db->select('count(students.gender) gender')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->where('student_session.session_id', $this->current_session);
    ...
    public function activestudents_female($class_id) 
    {
      $this->db->select('count(students.gender) gender')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
    ...
    public function allactivestudents($class_id) 
    {
      $this->db->select('count(students.id) total')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->where('student_session.session_id', $this->current_session);
    ...
    {
        $stu ="SELECT * FROM `students` ORDER BY id DESC LIMIT 1";
        $rs=$this->db->query($stu);
        return $rs->result_array();

    }
    
    
      public function getmarksheetdetails($id)
    {
        $this->db->select('a.*,b.*,c.*,d.* FROM students AS a, userresult_marksheet_log AS b, exam_group_class_batch_exams as c, exam_groups as d');
        $this->db->where('a.id=b.userid');
        $this->db->where('b.userid', $id);
        $this->db->where('b.exam_id=c.id');
        $this->db->where('c.exam_group_id =d.id');
        $query = $this->db->get();
        return $query->result_array();
    }
}
