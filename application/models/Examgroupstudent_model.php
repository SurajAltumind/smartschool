...
class Examgroupstudent_model extends CI_Model
{
    ...
    public function searchExamStudents($exam_group_id, $exam_id, $class_id, $section_id, $session_id)
    {
        ...
        $sql   = "SELECT  DISTINCT exam_group_class_batch_exam_students.id as `exam_group_class_batch_exam_student_id`,exam_group_class_batch_exam_students.roll_no as `exam_roll_no`, students.admission_no , students.id as `student_id`, students.roll_no,students.admission_date,students.firstname, students.lastname,students.image, students.mobileno, students.email ,students.state , students.city , students.pincode , students.religion,students.dob ,students.current_address, students.permanent_address,students.category_id, IFNULL(categories.category, '') as `category`, students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name, students.guardian_relation,students.guardian_phone,`classes`.`class`,students.guardian_address,students.is_active,`students`.`father_name`,`students`.`gender`,userresult_marksheet_log.published_date 
        ...
        LEFT JOIN `userresult_marksheet_log` ON students.id = `userresult_marksheet_log`.`userid`

        
        WHERE  exam_group_class_batch_exam_id=" . $this->db->escape($exam_id) . " AND students.is_active='yes' AND student_session.class_id=" . $this->db->escape($class_id) . " and student_session.section_id=" . $this->db->escape($section_id) . "  AND student_session.session_id=" . $this->db->escape($session_id).  "GROUP BY students.admission_no";
    ...
    public function examGroupSubjectResult4($exam_subject_id, $class_id, $section_id, $session_id)
    {
        $sql = "SELECT IFNULL(exam_group_exam_results.id, 0) as exam_group_exam_result_id,IFNULL(exam_group_exam_results.attendence,'') as `exam_group_exam_result_attendance`,IFNULL(exam_group_exam_results.get_marks,'') as `exam_group_exam_result_get_marks`,IFNULL(exam_group_exam_results.note,'') as `exam_group_exam_result_note`,IFNULL(exam_group_exam_results.reading_marks,'') as `exam_group_exam_result_reading_marks`,IFNULL(exam_group_exam_results.writing_marks,'') as `exam_group_exam_result_writing_marks`,IFNULL(exam_group_exam_results.project_marks,'') as `exam_group_exam_result_project_marks`,IFNULL(exam_group_exam_results.sliptest_marks,'') as `exam_group_exam_result_sliptest_marks`,
        IFNULL(exam_group_exam_results.summative_exam_marks,'') as `exam_group_exam_result_summative_exam_marks`,IFNULL(exam_group_exam_results.faone_fatwo_marks,'') as `exam_group_exam_result_faone_fatwo_marks`, exam_group_class_batch_exam_students.id as `exam_group_class_batch_exam_students_id`,exam_group_class_batch_exam_students.roll_no as `exam_roll_no`,exam_group_class_batch_exam_subjects.*, IFNULL(exam_group_exam_results.get_marks,'') AS TotalMarks,subjects.name,subjects.code,subjects.type,students.admission_no , students.id as `student_id`, students.roll_no,students.admission_date,students.firstname, students.lastname,students.image, students.mobileno, students.email ,students.state , students.city , students.pincode , students.religion,students.dob ,students.current_address, students.permanent_address,students.category_id, IFNULL(categories.category, '') as `category`, students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name, students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active,`students`.`father_name`,`students`.`gender` FROM `exam_group_class_batch_exam_subjects` INNER JOIN exam_group_class_batch_exams on exam_group_class_batch_exams.id=exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id INNER JOIN subjects on subjects.id=exam_group_class_batch_exam_subjects.subject_id INNER JOIN exam_group_class_batch_exam_students on exam_group_class_batch_exam_students.exam_group_class_batch_exam_id=exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id INNER join student_session on student_session.id=exam_group_class_batch_exam_students.student_session_id LEFT join exam_group_exam_results on exam_group_exam_results.exam_group_class_batch_exam_subject_id=exam_group_class_batch_exam_subjects.id and exam_group_exam_results.exam_group_class_batch_exam_student_id=exam_group_class_batch_exam_students.id  INNER JOIN students on students.id=student_session.student_id LEFT JOIN `categories` ON `students`.`category_id` = `categories`.`id`  WHERE exam_group_class_batch_exam_subjects.id=" . $this->db->escape($exam_subject_id) . " and  student_session.class_id=" . $this->db->escape($class_id) . " and student_session.section_id=" . $this->db->escape($section_id) . " and exam_group_class_batch_exams.formative_exam_status=2 and student_session.session_id=" . $this->db->escape($session_id) . "  ORDER BY students.id asc";
    ...
    public function studentExams($student_session_id)
    {
     ...
     public function searchExamStudentsFormative($exam_group_id, $exam_id, $class_id, $section_id, $session_id)
    {
        $sql = "SELECT exam_group_exam_results.reading_marks,exam_group_exam_results.writing_marks,exam_group_exam_results.project_marks,exam_group_exam_results.sliptest_marks,
exam_groups.name,exam_groups.exam_type,exam_group_class_batch_exams.exam,exam_group_class_batch_exams.session_id,students.admission_no,students.roll_no,students.firstname,students.lastname,sessions.session,subjects.name FROM exam_groups as exam_groups,exam_group_class_batch_exams as exam_group_class_batch_exams,students as students,sessions as session,subjects as subjects,exam_group_class_batch_exam_students as exam_group_class_batch_exam_students,exam_group_exam_results as exam_group_exam_results,sessions as sessions WHERE exam_groups.id=24 AND exam_group_class_batch_exams.exam_group_id=exam_groups.id AND sessions.id=18 AND exam_group_exam_results.formative_exam_marks_status='0' AND exam_group_class_batch_exams.exam='F.A-1'";
