...
class Examresult_model extends CI_Model {
    ...
    public function getExamResults($exam_id, $post_exam_group_id, $students, $startDate, $endDate, $StudentID) {
        ...
        foreach ($students as $student_key => $student_value) {
            ...
            if ($exam_connection) {
                ...
                $student['exam_result_cocarricular'] = $this->getCoCarricularSubjectStudentResultByExam($exam_id, $exam_group_class_batch_exam_student->id);
                $student['AnnualMarksheet'] = $this->getAnnualMarksheetData($exam_id, $exam_group_class_batch_exam_student->id);
                $student['formativeexamstotal'] = $this->getformativeexamstotal($exam_id, $exam_group_class_batch_exam_student->id);
                $student['TotalAttendanceDay'] = $this->getTotalAttendanceDay($exam_id, $exam_group_class_batch_exam_student->id, $startDate, $endDate, $StudentID);
                $student['TotalPresentDay'] = $this->getTotalPresentDay($exam_id, $exam_group_class_batch_exam_student->id,$startDate, $endDate, $StudentID);
                $result['students'][] = $student;
            }
        }
        return $result;
    ...
    public function getStudentResultByExam($exam_id, $student_id) {
        $sql = "SELECT exam_group_class_batch_exam_subjects.*,exam_group_exam_results.exam_group_class_batch_exam_student_id,exam_group_exam_results.exam_group_class_batch_exam_subject_id,exam_group_exam_results.id as `exam_group_exam_results_id`,exam_group_exam_results.attendence,exam_group_exam_results.get_marks,exam_group_exam_results.reading_marks,exam_group_exam_results.writing_marks,exam_group_exam_results.project_marks,exam_group_exam_results.sliptest_marks,exam_group_exam_results.summative_exam_marks,exam_group_exam_results.faone_fatwo_marks,exam_group_exam_results.note,subjects.name,subjects.code FROM `exam_group_class_batch_exam_subjects` inner JOIN exam_group_exam_results on exam_group_exam_results.exam_group_class_batch_exam_subject_id=exam_group_class_batch_exam_subjects.id INNER JOIN exam_group_class_batch_exam_students on exam_group_exam_results.exam_group_class_batch_exam_student_id=exam_group_class_batch_exam_students.id and exam_group_class_batch_exam_students.id=" . $this->db->escape($student_id) . " INNER JOIN subjects on subjects.id=exam_group_class_batch_exam_subjects.subject_id WHERE exam_group_class_batch_exam_subjects.exam_subject_type=1 AND exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id=" . $this->db->escape($exam_id);
        $query = $this->db->query($sql);
        return $query->result();
    }
    
    
      public function getAnnualMarksheetData($exam_id, $student_id) 
    {
        $sql = "SELECT a.*,b.name,b.code FROM exam_result_annually as a, subjects as b WHERE a.subjectId=b.id AND a.studentId='1784' AND a.examId='75' ";
        $query = $this->db->query($sql);
        return $query->result();
    }

       public function getformativeexamstotal($exam_id, $student_id)
    {
        $sql = "SELECT SUM(exam_group_exam_results.get_marks) as TotalMarks,exam_group_exam_results.exam_group_class_batch_exam_student_id,exam_group_exam_results.exam_group_class_batch_exam_subject_id,exam_group_class_batch_exam_subjects.*,exam_group_exam_results.exam_group_class_batch_exam_student_id,exam_group_exam_results.exam_group_class_batch_exam_subject_id,exam_group_exam_results.id as `exam_group_exam_results_id`,exam_group_exam_results.attendence,exam_group_exam_results.get_marks,exam_group_exam_results.reading_marks,exam_group_exam_results.writing_marks,exam_group_exam_results.project_marks,exam_group_exam_results.sliptest_marks,exam_group_exam_results.summative_exam_marks,exam_group_exam_results.faone_fatwo_marks,exam_group_exam_results.note,subjects.name,subjects.code FROM `exam_group_class_batch_exam_subjects` inner JOIN exam_group_exam_results on exam_group_exam_results.exam_group_class_batch_exam_subject_id=exam_group_class_batch_exam_subjects.id INNER JOIN exam_group_class_batch_exam_students on exam_group_exam_results.exam_group_class_batch_exam_student_id=exam_group_class_batch_exam_students.id  INNER JOIN subjects on subjects.id=exam_group_class_batch_exam_subjects.subject_id WHERE exam_group_exam_results.formative_exam_marks_status=2 AND exam_group_class_batch_exam_subjects.exam_subject_type=2";
        $query = $this->db->query($sql);
        return $query->result();  
    }

    public function getTotalAttendanceDay($exam_id,$student_id,$startDate,$endDate,$StudentID)
    {
        $sql = "SELECT COUNT(b.id) AS totaldayschool,a.firstname,a.lastname,b.date,b.attendence_type_id,c.session_id FROM students AS a, student_attendences AS b, student_session AS c WHERE b.attendence_type_id!=5  AND a.id=c.student_id AND b.student_session_id=c.id AND c.student_id='$StudentID' AND b.date BETWEEN '$startDate' AND '$endDate' ";
        $query = $this->db->query($sql);
        return $query->result();
    }

    public function getTotalPresentDay($exam_id,$student_id,$startDate,$endDate,$StudentID)
    {
        $sql = "SELECT COUNT(b.id) AS totalPrsentDay,a.firstname,a.lastname,b.date,b.attendence_type_id,c.session_id FROM students AS a, student_attendences AS b, student_session AS c WHERE a.id=c.student_id AND b.student_session_id=c.id AND c.student_id='$StudentID' AND b.attendence_type_id=1 AND b.date BETWEEN '$startDate' AND '$endDate' ";
        $query = $this->db->query($sql);
        return $query->result();
    }

    public function getCoCarricularSubjectStudentResultByExam($exam_id, $student_id)
    {
        $sql = "SELECT exam_group_class_batch_exam_subjects.*,exam_group_exam_results.id as `exam_group_exam_results_id`,exam_group_exam_results.attendence,exam_group_exam_results.get_marks,exam_group_exam_results.get_marks AS Cocarricularsum,exam_group_exam_results.reading_marks,exam_group_exam_results.writing_marks,exam_group_exam_results.project_marks,exam_group_exam_results.sliptest_marks,exam_group_exam_results.summative_exam_marks,exam_group_exam_results.faone_fatwo_marks,exam_group_exam_results.note,subjects.name,subjects.code FROM `exam_group_class_batch_exam_subjects` inner JOIN exam_group_exam_results on exam_group_exam_results.exam_group_class_batch_exam_subject_id=exam_group_class_batch_exam_subjects.id INNER JOIN exam_group_class_batch_exam_students on exam_group_exam_results.exam_group_class_batch_exam_student_id=exam_group_class_batch_exam_students.id and exam_group_class_batch_exam_students.id=" . $this->db->escape($student_id) . " INNER JOIN subjects on subjects.id=exam_group_class_batch_exam_subjects.subject_id WHERE exam_group_class_batch_exam_subjects.exam_subject_type=2 AND exam_group_class_batch_exam_subjects.exam_group_class_batch_exams_id=" . $this->db->escape($exam_id);
        $query = $this->db->query($sql);
        return $query->result();
    }
