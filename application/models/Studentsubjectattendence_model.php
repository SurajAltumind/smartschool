...
class Studentsubjectattendence_model extends CI_Model {
    ...
    public function searchAttendenceClassSection($class_id, $section_id, $subject_timetable_id, $date, $attendanceFlag) 
    {
       ...
       if($attendanceFlag==1)
       {
        ...
        if ($attendanceFlag == 2) {
            ...
            // $sql = "SELECT a.id,a.admission_no,a.roll_no,a.firstname,a.middlename,a.lastname,b.created_at,b.login_time,b.logout_time,c.id FROM students AS a, conferences_history AS b,student_session AS c,student_subject_attendances AS d  WHERE a.id=b.student_id AND a.id=c.student_id AND c.class_id=" . $this->db->escape($class_id) . " AND c.section_id =" . $this->db->escape($section_id) . " AND c.session_id=" . $this->db->escape($this->current_session) . " AND c.id=d.student_session_id and d.subject_timetable_id=" . $this->db->escape($subject_timetable_id) . "  AND a.is_active='yes' GROUP BY a.id ";
            // $query = $this->db->query($sql);
            // return $query->result_array();

            $sql = "SELECT a.id,a.admission_no,a.roll_no,a.firstname,a.middlename,a.lastname,b.studentautoate_created_date,b.studentautoate_login_time,b.studentautoate_logout_time,c.id,e.title FROM students AS a, student_automatic_attendance AS b,student_session AS c,student_subject_attendances AS d,conferences AS e  WHERE a.id=b.studentautoate_student_id AND a.id=c.student_id AND c.class_id=" . $this->db->escape($class_id) . " AND c.section_id =" . $this->db->escape($section_id) . " AND c.session_id=" . $this->db->escape($this->current_session) . " AND c.id=d.student_session_id AND a.is_active='yes' AND b.studentautoate_conference_id=e.id AND b.studentautoate_created_date='$date' GROUP BY a.id ";
            $query = $this->db->query($sql);
            return $query->result_array();
        }
    }
