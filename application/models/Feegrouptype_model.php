...
class Feegrouptype_model extends CI_Model {
     ...
     */
       ...
       public function remove($id) {
        ...
        $num_rows = $this->db->count_all_results('fee_groups_feetype');
        if ($num_rows == 1) {
            $updateDelStatusFeeSessionGroup = array(
                'fee_session_groups_delete_status' => 0,
            );
            $this->db->where('id', $fee_session_group_id);
            $result = $this->db->update('fee_session_groups', $updateDelStatusFeeSessionGroup);
        }

        
        $updateDelStatusFeeGroupFeeType = array(
            'fee_groups_feetype_delete_status' => 0,
        );
        $this->db->where('id', $id);
        $result = $this->db->update('fee_groups_feetype', $updateDelStatusFeeGroupFeeType);	
